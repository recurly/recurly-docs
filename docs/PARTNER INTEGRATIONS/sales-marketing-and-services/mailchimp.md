...
2. **Log into Recurly**: Navigate to the Integrations section and select [MailChimp][3].
...
- **ACCOUNT_ID**: Recurly account_code.
- **FNAME**: Account first name.
- **LNAME**: Account last name.
- **COMPANY**: Account company name.
- **PLAN_CODE**: Subscription plan code (if the account is a subscriber).
- **PLAN_NAME**: Subscription plan name (if the account is a subscriber).
- **IN_TRIAL**: Indicates if the subscriber is in a trial period.
- **SUBSCRIBER**: Denotes if the account has a subscription.
- **SUB_SINCE**: Date the subscription began.

### Disabling the integration

If you ever need to halt the synchronization, you can easily disable it from [the MailChimp Integration page][3] in Recurly. 
