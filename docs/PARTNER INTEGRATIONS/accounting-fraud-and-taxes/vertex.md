...
In addition, Recurly's integration with Vertex comes with the ability to breakdown taxes based on the jurisdiction type.  This setting, **Communication Tax Breakdown**, can be found when creating or editing an invoice, and is only visible when Vertex is turned on for your site.  
