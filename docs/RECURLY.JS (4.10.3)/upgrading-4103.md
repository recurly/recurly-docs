...
Your `recurly.token` call may also need to be updated. It is now necessary to pass a reference to your checkout `<form>` that contains the hosted fields. Your callback function requires no modification. More info is available in the [Getting a Token](https://docs.recurly.com/v1.2/docs/getting-a-token-4103#/) section.
