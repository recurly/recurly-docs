...
* While not natively supported in Recurly.js, if you want ApplePay to function in Chrome you may load the [latest version of Apple in your environment](https://developer.apple.com/documentation/applepayontheweb/loading-the-latest-version-of-apple-pay-js#Load-the-auto-updating-version-of-the-SDK). We will have further documentation of this process in our [ApplePay Recurly.js documentation](https://docs.recurly.com/recurly-subscriptions/docs/apple-pay#non-safari-browsers). This is not yet supported in Recurly Checkout UI.
...
* For **return customers**, please use Recurly.js even if an existing account code or billing info exists, as Recurly.js handles collecting device-related data (cryptograms) necessary for customer-initiated transactions through Apple Pay. [See documentation for more information](https://docs.recurly.com/recurly-subscriptions/docs/3d-secure#re-authenticating-existing-billing-information).
...
### Using Apple Recurring Requests
...
2. **Generate Merchant ID and Payment Processing Keys** Recurly Support will provide the keys you need for Apple Pay integration.

3. **Generate Certificate Signing Requests (CSRs)** These CSRs are required to create the **Apple Pay Payment** and **Merchant Identity** certificates. Each CSR is unique to your merchant account, so allow several business days for these files to be generated by Recurly Support. **You must use the Recurly-provided CSRs** to ensure the integration functions correctly.
