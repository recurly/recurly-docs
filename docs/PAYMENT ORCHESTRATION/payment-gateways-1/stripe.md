...
- **Single Subscription Plan per transaction:&#x20;**&#x54;o avoid declines on renewals, especially when mandates are involved with India card payments, for best renewal auth rates, a single plan per invoice is supported. Adding multiple subscriptions on a single transaction could result in renewal declines and mandate cancellations by the consumer.
