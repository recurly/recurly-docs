...
.rc-compare-label-sprawl { background: rgba(255,130,0,0.12); color: #b35a00; }
.rc-compare-label-segment { background: rgba(255,215,6,0.18); color: #7a6500; }
...
<div class="rc-guide">
  ...
  <div class="rc-content-wrap">
    ...
    <div class="rc-hero">
      ...
      <div class="rc-lp-hero-title"><h1>Price segmentation — Overview</h1></div>
    ...
    <details class="rc-sticky-nav-wrap" open>
      ...
      <div class="rc-nav-drawer"><div class="rc-nav-drawer-inner"><div class="rc-nav-links">
        ...
        <a href="https://docs.recurly.com/recurly-subscriptions/docs/navigate-acquire-pricing-plans-201-segmentation" class="rc-sticky-link rc-sticky-link-active">
          <img src="https://files.readme.io/070e914d23dead09604d5f96b8769c88b8aae704ebd4505415e5854011030110-Black_Navigate_Home_Pin_1.png" alt=""> Segmentation: Overview
        </a>
        ...
        <!-- Back to Path Start — always last, circle arrow -->
    ...
    <div class="rc-lp-section" id="what-are-price-segments">
      <h2>🏷️ What are price segments in Recurly?</h2>
      <p>A price segment is a named price point within a plan that you assign to a subscriber at the moment their subscription is created. When Recurly calculates what that subscriber owes each billing period, it uses the price associated with their segment code — not the plan's default price.</p>
      <p>Everything else about the subscription stays the same regardless of which segment the subscriber is on. The billing interval, add-on configuration, dunning campaign assignment, trial setup — all defined once at the plan level, all shared across every segment. <strong>Price segments change what a subscriber pays. They don't change how the subscription is structured.</strong></p>
      ...
      <div class="rc-stat-strip">
        <div class="rc-stat-tile">
          ...
          <div class="rc-stat-tile-context">Price segments are configured on the plan itself, alongside the default price. Any number of segments can be added to a single plan.</div>
        </div>
        <div class="rc-stat-tile">
          <div class="rc-stat-tile-num">At creation</div>
          <div class="rc-stat-tile-label">When the code is assigned</div>
          <div class="rc-stat-tile-context">The segment code is passed when the subscription is created — via your checkout flow or API call. It can be updated later via the API if a subscriber moves between segments.</div>
        </div>
        <div class="rc-stat-tile">
          <div class="rc-stat-tile-num">Per-currency</div>
          <div class="rc-stat-tile-label">How prices are set</div>
          <div class="rc-stat-tile-context">Each segment has its own price in each currency you support. Segment prices are set manually — Recurly does not auto-convert between currencies.</div>
        </div>
      </div>
    </div>

    <!-- The plan sprawl problem -->
    <div class="rc-lp-section" id="plan-sprawl">
      <h2>📋 The plan sprawl problem — and how segmentation solves it</h2>
      <p>Without price segments, every distinct price point requires a separate plan. A business serving four geographic markets at market-specific prices needs four plans — even if the only thing that differs between them is the number on the invoice. Multiply that by billing intervals, trial configurations, and product tiers, and you can easily accumulate dozens or hundreds of plans that are functionally identical except for the price.</p>
      <p>Plan sprawl isn't just an operational inconvenience. It makes analytics harder (churn by plan becomes meaningless when one "plan" is really fifteen), dunning management more complex (each plan needs its own campaign assignment review), and plan code conventions break down. Price segments collapse all of that back into one plan.</p>

      <div class="rc-compare-grid">
        <div class="rc-compare-card">
          <div class="rc-compare-label rc-compare-label-sprawl">Without segments</div>
          <h4>🗂️ Plan-per-price-point approach</h4>
          <ul>
            <li>US market plan, EU plan, UK plan, APAC plan — all identical except for price</li>
            <li>A/B pricing test requires two separate plans and split traffic routing</li>
            <li>Grandfathered cohorts each need their own plan to preserve their price</li>
            <li>Analytics filters by plan — not by the actual pricing variable being measured</li>
            <li>Adding a new market means creating, configuring, and maintaining another plan</li>
          </ul>
        </div>
        <div class="rc-compare-card">
          <div class="rc-compare-label rc-compare-label-segment">With price segments</div>
          <h4>🎯 One-plan approach</h4>
          <ul>
            <li>One plan with segment codes: <code>us</code>, <code>eu</code>, <code>uk</code>, <code>apac</code> — each with its own price</li>
            <li>A/B test runs on the same plan — pass <code>price_a</code> or <code>price_b</code> at subscription creation</li>
            <li>Grandfathered subscribers keep their segment code; new subscribers get the current one</li>
      ...
      <div class="rc-callout rc-callout-tip">
        ...
        <div class="rc-callout-body">
          <strong>When plan sprawl is already advanced</strong>
          <p>If your Recurly account already has many near-identical plans, price segmentation won't retroactively consolidate them — existing subscriptions stay on their current plan codes. But you can stop adding to the problem: any new market, cohort, or test going forward can use segments on a single plan instead of spinning up another one. Build the cleaner foundation now; consolidation can come later as subscriptions naturally turn over.</p>
        </div>
      </div>
    </div>
    ...
    <div class="rc-lp-section" id="use-cases">
      <h2>🎯 Three use cases where segments earn their complexity</h2>
      <p>Price segments add a layer of configuration that's worth it in three specific situations. Outside these, the default plan price — or a separate plan — is usually simpler.</p>

      <div class="rc-card-grid">
        <div class="rc-feature-card">
          <div class="rc-feature-icon">🌍</div>
          <h4>Geographic pricing</h4>
          <p>Charge $12/month in the US, €9.99/month in Germany, and £8.99/month in the UK — all on the same plan. Each segment has its own currency-specific price. Recurly's Hosted Payment Page can detect location via geo-IP and pre-select the correct segment, or you can pass the segment code explicitly via API based on your own location logic.</p>
          <span class="rc-feature-tag">Geo-pricing</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon">🧪</div>
          <h4>A/B price testing</h4>
          <p>Test $29/month against $39/month without creating a second plan or splitting your reporting between two plan codes. Route new subscribers to <code>price_a</code> or <code>price_b</code> at subscription creation, track conversion and retention externally by segment code, and roll out the winner by updating which code your checkout passes going forward.</p>
          <span class="rc-feature-tag">Price testing</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon">🔒</div>
          <h4>Grandfathered pricing</h4>
          <p>When you raise prices for new subscribers, existing subscribers can keep their original price indefinitely by retaining their original segment code. New subscribers are created with the current segment. No plan migration, no versioning complexity — the same plan accommodates both cohorts simultaneously.</p>
          <span class="rc-feature-tag">Cohort pricing</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon">📣</div>
      ...
      <div class="rc-callout rc-callout-caution">
        ...
        <div class="rc-callout-body">
          ...
          <p>Recurly's built-in analytics and reporting do not segment performance by price segment code. If you're running an A/B price test or measuring conversion rates by geography, you'll need to track segment code alongside subscription data in an external analytics tool or BI system. This is the most important thing to plan for before activating segments — don't run a test you can't measure.</p>
        </div>
      </div>
    </div>
