---
title: 'Account Hierarchy: Overview'
deprecated: false
hidden: true
metadata:
  robots: index
...
.rc-compare-label-child { background: rgba(13,13,11,0.08); color: var(--offblack); }
...
@media(max-width:768px){
  ...
  .rc-card-grid, .rc-compare-grid { grid-template-columns: 1fr; }
...
<div class="rc-guide">
  ...
  <div class="rc-content-wrap">
    ...
    <div class="rc-hero">
      ...
      <div class="rc-lp-hero-title"><h1>Account hierarchy — Overview</h1></div>
    ...
    <details class="rc-sticky-nav-wrap" open>
      ...
      <div class="rc-nav-drawer"><div class="rc-nav-drawer-inner"><div class="rc-nav-links">
        ...
        <a href="https://docs.recurly.com/recurly-subscriptions/docs/navigate-acquire-pricing-plans-201-hierarchy" class="rc-sticky-link rc-sticky-link-active">
          <img src="https://files.readme.io/070e914d23dead09604d5f96b8769c88b8aae704ebd4505415e5854011030110-Black_Navigate_Home_Pin_1.png" alt=""> Hierarchy: Overview
        </a>
    ...
    <div class="rc-lp-section" id="what-is-hierarchy">
      <h2><i class="fa-solid fa-sitemap rc-fa-section"></i> What is Account Hierarchy in Recurly?</h2>
      <p>Account Hierarchy is Recurly's way of modeling complex organizational billing relationships. You create a <strong>parent account</strong> to represent the top-level entity — a headquarters, a holding company, a reseller — and link one or more <strong>child accounts</strong> to it, each representing a subsidiary, location, department, or end customer.</p>
      <p>The relationship is visible in the Recurly admin, exposed through the API, and reflected on invoices. But the most commercially important decision it unlocks is billing responsibility: for each child account, you decide whether that account pays its own invoices, or whether all charges roll up to the parent.</p>

      <div class="rc-stat-strip">
        <div class="rc-stat-tile">
          <div class="rc-stat-tile-num">Many-to-one</div>
          <div class="rc-stat-tile-label">Hierarchy structure</div>
          <div class="rc-stat-tile-context">A parent can have unlimited child accounts. Each child has exactly one parent. The relationship can be created or changed at any time via the admin UI or API.</div>
        </div>
        <div class="rc-stat-tile">
          <div class="rc-stat-tile-num">Per-child</div>
          <div class="rc-stat-tile-label">Billing responsibility</div>
          <div class="rc-stat-tile-context">Billing responsibility is configured per child account — not at the parent level. One parent can have some children billing to the parent and others managing their own invoices.</div>
        </div>
        <div class="rc-stat-tile">
          <div class="rc-stat-tile-num">Parent's address</div>
          <div class="rc-stat-tile-label">Tax treatment</div>
          <div class="rc-stat-tile-context">When a child bills to the parent, Recurly uses the parent's billing address for tax calculations on those invoices. Plan for this in multi-jurisdiction setups.</div>
        </div>
      </div>
    </div>
    ...
    <div class="rc-lp-section" id="billing-modes">
      <h2><i class="fa-solid fa-scale-balanced rc-fa-section"></i> The two billing modes — and when to use each</h2>
      <p>This is the decision that matters most in any Account Hierarchy setup. Every child account is configured in one of two billing modes. The choice determines where invoices live, who holds the payment method, and how your finance team reconciles charges.</p>
      ...
      <div class="rc-compare-grid">
        <div class="rc-compare-card">
          ...
          <ul>
            <li>All invoices, charges, credits, and payment information belong to the parent account</li>
            <li>Child account has no payment method on file — parent pays everything</li>
            <li>Ideal for enterprise deals where procurement controls payment centrally</li>
            <li>Enables Invoice Rollup — all child charges consolidated into one parent invoice</li>
            <li>Tax calculated using parent's billing address, regardless of child location</li>
            <li>Coupons and credits applied to the parent flow across all billed-to-parent children</li>
          </ul>
        </div>
        <div class="rc-compare-card">
          <div class="rc-compare-label rc-compare-label-child">Bill to self</div>
          ...
          <ul>
            ...
            <li>Tax calculated using child's own billing address</li>
      ...
      <div class="rc-callout rc-callout-tip">
        ...
        <div class="rc-callout-body">
          ...
          <p>A single parent account can have children in both billing modes simultaneously. A global enterprise might have its North America divisions billing to the parent while its APAC entities manage their own invoices. Billing responsibility is configured per child — you're not locked into one model across the whole hierarchy.</p>
        </div>
      </div>
    </div>
    ...
    <div class="rc-lp-section" id="invoice-rollup">
      <h2><i class="fa-solid fa-layer-group rc-fa-section"></i> Invoice Rollup — consolidated billing for enterprise accounts</h2>
      <p>Invoice Rollup is the feature that makes Account Hierarchy commercially compelling for enterprise sales. Rather than a parent account receiving a separate invoice per child subscription, Invoice Rollup consolidates line items from all billed-to-parent child accounts into a single parent invoice — one document, one payment, full line-item transparency.</p>

      <div class="rc-card-grid">
        <div class="rc-feature-card">
          <div class="rc-feature-icon"><i class="fa-regular fa-file-lines"></i></div>
          <h4>One invoice, many accounts</h4>
          <p>All recurring charges, one-time charges, and credits from billed-to-parent children appear on the parent's invoice. Each line item shows which child account it originated from — full transparency without invoice fragmentation.</p>
          <span class="rc-feature-tag">Consolidated billing</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon"><i class="fa-regular fa-calendar-days"></i></div>
          <h4>Powered by Calendar Billing</h4>
          <p>Invoice Rollup requires Calendar Billing — Recurly's feature that aligns all child billing dates to match the parent's billing date. Without aligned dates, consolidation isn't possible. Calendar Billing must be enabled before Invoice Rollup can work.</p>
          <span class="rc-feature-tag">Calendar Billing dependency</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon"><i class="fa-solid fa-headset"></i></div>
          <h4>Requires support activation</h4>
          <p>Invoice Rollup is not enabled by default. To use Account Hierarchy with Invoice Rollup, you must contact Recurly Support to activate it on your site. Plan this into your implementation timeline — activation is not self-serve.</p>
          <span class="rc-feature-tag">Support activation required</span>
        </div>
        <div class="rc-feature-card">
          <div class="rc-feature-icon"><i class="fa-solid fa-magnifying-glass"></i></div>
          <h4>Full API visibility</h4>
          <p>All charges are retrievable via the Recurly API through the invoices and line items endpoints. Each line item shows which account the charge originated from and which account it was billed to. Both API v2 and v3 support hierarchy data.</p>
          <span class="rc-feature-tag">API accessible</span>
        </div>
      </div>
      ...
      <div class="rc-callout rc-callout-caution">
        ...
        <div class="rc-callout-body">
          <strong>Invoice Rollup is not suitable for all hierarchy setups</strong>
          <p>If child accounts are in different tax jurisdictions from the parent, rolling up to a single invoice taxes all charges at the parent's address. For businesses with complex multi-jurisdiction tax requirements, evaluate carefully before activating Invoice Rollup — the billing simplicity may create tax compliance complexity.</p>
        </div>
      </div>
    </div>
    ...
    <div class="rc-lp-section" id="use-cases">
      ...
      <p>Account Hierarchy adds configuration overhead. It's worth it in these specific situations — and unnecessary overhead everywhere else.</p>

      <div class="rc-card-grid">
        <div class="rc-feature-card">
          <div class="rc-feature-icon"><i class="fa-solid fa-industry"></i></div>
          <h4>Multi-location enterprise customers</h4>
          <p>A restaurant chain, retail franchise, or healthcare network with dozens of locations — each needing its own subscription and service configuration, but all billed to a central procurement account. Hierarchy makes this a single billing relationship instead of dozens of unconnected accounts.</p>
        ...
        <div class="rc-feature-card">
          ...
          <p>A digital agency managing subscriptions on behalf of multiple end clients. The agency (parent) is financially responsible; the clients (children) each have their own subscription and data. The agency sees consolidated charges; clients see line-item detail.</p>
        ...
        <div class="rc-feature-card">
          ...
          <p>One payer (parent) covers subscriptions for multiple household members (children), each with their own account and access credentials. The parent pays a single consolidated bill; children manage their own profile and usage independently.</p>
        ...
        <div class="rc-feature-card">
          ...
          <p>A holding company with several operating subsidiaries, each needing separate subscriptions and invoices for internal accounting — but with the parent company holding ultimate payment responsibility and needing consolidated billing visibility.</p>
    ...
    <!-- Path nav -->
    <div class="rc-lp-nav">
      <a class="rc-btn-prev">← Segmentation</a>
      <span class="rc-lp-nav-indicator">1 of 3 · Account Hierarchy</span>
      <a href="https://docs.recurly.com/recurly-subscriptions/docs/navigate-acquire-pricing-plans-201-hierarchy-configure" class="rc-btn-path">Next: How to configure →</a>
    </div>
