---
title: Getting started v2999-01-01
excerpt: >-
  Learn how to get started with the Recurly API, including versioning, error
...
This guide walks you through the essential concepts you'll need to work with the Recurly API. We'll cover versioning, error handling, pagination, and rate limits — everything you need to build a stable integration.

## API versioning

The Recurly API uses date-based versioning to give you stability while we continue improving the platform. This means your code won't break when we ship new features.
...
Accept: application/vnd.recurly.v2999-01-01+json
```
...
Versions follow a `YYYY-MM-DD` format (for example, `v2999-01-01`), which reflects the approximate date the changes were released.
...
**Client library releases by language:**
...
Client libraries follow semantic versioning. For example, Ruby client library version 3.18.1 represents major version `3`, minor version `18` with patch `1`.

We recommend updating to the latest version of both the API and your client library regularly. Later versions are more performant and include important bug fixes. See the [changelog](#changelog) for a complete list of improvements in this version.
...
```json
{
  "error": {
    ...
    "acceptable_accept_headers": [
      ...
      "application/vnd.recurly.v2999-01-01"
    ]
  }
}
```
...
```json
{
  "object": "list",
  "has_more": true,
  "next": "https://api.recurly.com/v2999-01-01/accounts?limit=200&cursor=1234",
  "data": [
...
```javascript
let allRecords = [];
let nextUrl = 'https://api.recurly.com/v2999-01-01/accounts';

while (nextUrl) {
  const response = await fetch(nextUrl, {
    headers: { 'Accept': 'application/vnd.recurly.v2999-01-01' }
  });
...
```
GET /v2999-01-01/accounts?limit=100&sort=updated_at&order=desc&begin_time=2024-01-01T00:00:00Z
```
...
```
HEAD /v2999-01-01/accounts?begin_time=2024-01-01T00:00:00Z
```
...
For a complete list of changes and improvements in this version, see the [changelog](https://recurly.com/developers/api/changelog.html#v2999-01-01).
