---
title: Getting started v2022-01-01
excerpt: >-
...
Versions follow a `YYYY-MM-DD` format (for example, `v2022-01-01`), which reflects the approximate date the changes were released.
...
We recommend updating to the latest version of both the API and your client library regularly. Later versions are more performant and include important bug fixes. See the [changelog](https://recurly.com/developers/api/changelog.html) for a complete list of improvements in the latest version.
...
```json
{
  "object": "list",
  "has_more": true,
  "next": "https://api.recurly.com/v2022-01-01/accounts?limit=200&cursor=1234",
  "data": [
...
```javascript
let allRecords = [];
let nextUrl = 'https://api.recurly.com/v2022-01-01/accounts';

while (nextUrl) {
  const response = await fetch(nextUrl, {
    headers: { 'Accept': 'application/vnd.recurly.v2022-01-01' }
  });
...
```
GET /v2022-01-01/accounts?limit=100&sort=updated_at&order=desc&begin_time=2024-01-01T00:00:00Z
```
...
```
HEAD /v2022-01-01/accounts?begin_time=2024-01-01T00:00:00Z
```
